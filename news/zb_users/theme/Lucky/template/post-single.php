...
<div class="post">
    <h1 class="post-title">
		...
		{if $user.ID>0}
			<a href="{$host}zb_system/admin/edit.php?act=ArticleEdt&id={$article.ID}" target="_blank">[编辑]</a>
		{/if}
					...
					<div class="qrcode-img qrCode_0" id="qrCode_0">
						<div class="qrcode-border box-size" style="border: 9.02px solid rgb(60, 175, 54">
							{if $zbp->Config('Lucky')->wx}
								<img class="qrcode-img qrCode_0" id="qrCode_0" src="{Lucky_Host($zbp->Config('Lucky')->wx)}" />
							{else}
								<img class="qrcode-img qrCode_0" id="qrCode_0" src="{Lucky_Host()}zb_users/theme/Lucky/style/image/wx.png" />
							{/if}
						</div>
					...
					<div class="qrcode-img qrCode_1" id="qrCode_1">
						<div class="qrcode-border box-size" style="border: 9.02px solid rgb(102, 153, 204">
							{if $zbp->Config('Lucky')->qq}
								<img class="qrcode-img qrCode_1" id="qrCode_1" src="{Lucky_Host($zbp->Config('Lucky')->qq)}" />
							{else}
								<img class="qrcode-img qrCode_1" id="qrCode_1" src="{Lucky_Host()}zb_users/theme/Lucky/style/image/qq.png" />
							{/if}
						</div>
						<p class="qrcode-tip">打赏</p>
					</div>
					<div class="qrcode-img qrCode_2" id="qrCode_2">
						<div class="qrcode-border box-size" style="border: 9.02px solid rgb(235, 95, 1">
							{if $zbp->Config('Lucky')->zfb}
								<img class="qrcode-img qrCode_2" id="qrCode_2" src="{Lucky_Host($zbp->Config('Lucky')->zfb)}" />
							{else}
								<img class="qrcode-img qrCode_2" id="qrCode_2" src="{Lucky_Host()}zb_users/theme/Lucky/style/image/zfb.png" />
							{/if}
						</div>
						<p class="qrcode-tip">打赏</p>
					</div>
        ...
        <div class="p_tags">
            <div class="tagcloud">
                <span>
                    ...
                    标签：{if $article.Tags}{foreach $article.Tags as $tag}<a href="{$tag.Url}" title="{$tag.Name}" class="label bg-primary">{$tag.Name}</a>{/foreach}{else}本文暂时没有添加标签{/if}
                </span>
            </div>
        </div>
    </div>
    <div class="p-authorinfo">
        <div class="p-copyright">
            {$zbp->Config('Lucky')->article_copyright}
            <p>
