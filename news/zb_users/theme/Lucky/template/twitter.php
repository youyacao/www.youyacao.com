	...
	<div class="twitter_main">
		<div class="twitter_avatar">
			{if $zbp->Config('Lucky')->avatar}
				<img src="{Lucky_Host()}zb_users/theme/Lucky/style/image/grey.gif" data-original="{Lucky_Host($zbp->Config('Lucky')->avatar)}" alt="{$article.Author.StaticName}"/>
			{else}
				<img src="{Lucky_Host()}zb_users/theme/Lucky/style/image/grey.gif" data-original="{Lucky_Host()}zb_users/theme/Lucky/style/image/avatar.png" alt="{$article.Author.StaticName}"/>
			{/if}
		</div>
		<div class="twitter_content">
			...
			{if $article->Metas->imgs[0] != null}
				<ul class="twitter_images">
					{foreach $article.Metas.imgs as $key => $values}
						...
						{/php}
						<li><a data-src="{Lucky_Host($values)}" href="{Lucky_Host($values)}" class="lightgallery_item"><img src="{Lucky_Host()}/zb_users/theme/Lucky/style/image/grey.gif" data-original="{$thumbImages}" alt="{$article.Title} 第{$key+1}张" /></a></li>
					{/foreach}
					<div class="clear"></div>
				</ul>
			{/if}
			...
			<div class="twitter_time">
				...
				<span class="twitter_t tra"><a href="javascript:void(0)" class="actio Whisper-like sf-praise-sdk" sfa="click" data-postid="{$sf_praise_sdk.postid}" data-value="1"><i class="fa fa-thumbs-up"></i> 赞(<span class="sf-praise-sdk" sfa="num" data-value="1" data-postid="{$sf_praise_sdk.postid}">{$sf_praise_sdk.value1}</span>)</a></span>
			</div>
		</div>
	</div>
</div>
