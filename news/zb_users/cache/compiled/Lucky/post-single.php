...
<div class="post">
    <h1 class="post-title">
		<?php if ($article->IsTop) { ?>[置顶]<?php } ?>
		<?php if ($user->ID>0) { ?>
			<a href="<?php  echo $host;  ?>zb_system/admin/edit.php?act=ArticleEdt&id=<?php  echo $article->ID;  ?>" target="_blank">[编辑]</a>
		<?php } ?>
		<a href="<?php  echo $article->Url;  ?>" rel="bookmark" title="<?php  echo $article->Title;  ?>"><?php  echo $article->Title;  ?></a>
	</h1>
    <div class="p_info">
        ...
        <span class="info_category info_ico">
			<i class="fa fa-folder-open"></i> <a href="<?php  echo $article->Category->Url;  ?>" title="<?php  echo $article->Category->Name;  ?>" rel="category tag"><?php  echo $article->Category->Name;  ?></a>
        </span>
        ...
        <div class="p_tags">
            <div class="tagcloud">
                <span>
                    ...
                    标签：<?php if ($article->Tags) { ?><?php  foreach ( $article->Tags as $tag) { ?><a href="<?php  echo $tag->Url;  ?>" title="<?php  echo $tag->Name;  ?>" class="label bg-primary"><?php  echo $tag->Name;  ?></a><?php }   ?><?php }else{  ?>本文暂时没有添加标签<?php } ?>
                </span>
            </div>
        </div>
    </div>
    <div class="p-authorinfo">
        <div class="p-copyright">
            <?php  echo $zbp->Config('Lucky')->article_copyright;  ?>
            <p>
                转载请注明 : 文章转载自 <i class="fa fa-angle-double-right"></i>&nbsp;
                <a href="<?php  echo $host;  ?>" rel="bookmark" title="作者：<?php  echo $article->Author->StaticName;  ?>">
                    <?php  echo $name;  ?>
                </a>
                <i class="fa fa-angle-double-right"></i>&nbsp;
                <a href="<?php  echo $article->Url;  ?>" rel="bookmark" title="本文固定链接：<?php  echo $article->Url;  ?>">
                    <?php  echo $article->Title;  ?>
                </a>
            </p>
            <p>
                本文标题：<?php  echo $article->Title;  ?>
            </p>
            <p>
                本文链接：<?php  echo $article->Url;  ?>
            </p>
        </div>
        <div class="clear">
        </div>
    </div>
	<?php if ($zbp->Config('Lucky')->gdjc=="a") { ?>
	<div class="p-recommend">
		<div class="p-recommend-title">更多精彩</div>
		<div class="p-recommend-all">
			<div class="recommend-left">
			<?php  foreach ( GetList(3,null,null,null,null,null,array('is_related'=>$article->ID)) as $key => $related) { ?>
			<?php  $i=$key+1;  ?>
				<li class="recommend-<?php  echo $i++;  ?>" ><span>[<?php  echo $related->Time('m-d');  ?>]</span><a href="<?php  echo $related->Url;  ?>" title="<?php  echo $related->Title;  ?>" rel="bookmark"><?php  echo $related->Title;  ?></a></li>
			<?php }   ?>
			</div>
			<div class="recommend-right">
			<?php  foreach ( GetList(6,null,null,null,null,null,array('is_related'=>$article->ID)) as $key => $related) { ?>
			<?php if ($key>2) { ?>
				<li class="recommend-<?php  echo $i++;  ?>" ><span>[<?php  echo $related->Time('m-d');  ?>]</span><a href="<?php  echo $related->Url;  ?>" title="<?php  echo $related->Title;  ?>" rel="bookmark"><?php  echo $related->Title;  ?></a></li>
			<?php } ?>
			<?php }   ?>
    ...
    <div class="r-pn-post">
        ...
        <div class="xia">
			<?php if ($article->Next) { ?>
				...
				<span>下一篇 :</span> <?php  echo $article->Next->Title;  ?></a>
			<?php }else{  ?>
				<a href="javascript:void(0)" class="next_p">
